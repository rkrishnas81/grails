...
def main():
    ...
    if stocks_matching_today:
        ...
        print("Stock   Rule Id   WinRate   Avg Next Open %")
        for stock, rule_id, winrate, avg_next_open in sorted(stocks_matching_today, key=lambda x: x[0]):
            print(f"{stock:<7} {rule_id:<8} {winrate:>7.2f}% {avg_next_open:>14.3f}%")
    else:
