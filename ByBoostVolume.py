...
def main() -> None:
    ...
    for t in tickers:
        ...
        # NextDayOpen% (backtest only)
        nextday_open_pct = np.nan
        if i + 1 < len(feats):
            c0 = safe(row_for_score.get("Close", np.nan))
            o1 = safe(feats.iloc[i + 1].get("Open", np.nan))
            if np.isfinite(c0) and np.isfinite(o1) and c0 != 0:
                nextday_open_pct = (o1 / c0 - 1) * 100
        ...
        scan_rows.append({
            ...
            "NextDayOpen%": (f"{nextday_open_pct:.2f}%" if np.isfinite(nextday_open_pct) else ""),
